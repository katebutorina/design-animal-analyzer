/**
 * Основной файл скрипта для приложения Designanimals с поддержкой мультиязычности
 */
...
/**
 * Получение элементов интерфейса с учетом текущего языка
 */
...
/**
 * Функция для установки правильного окончания для "проявлен" в русском языке
 * @param {string} gender - род существительного (masculine, feminine, neuter)
 */
...
/**
 * Позиционирует переключатель языков в двух последних ячейках сетки
 */
function positionLangSwitcher() {
    // CSS обеспечивает позиционирование
}
...
function updateMovementMetrics(x, y, speed) {
    ...
    // Try to determine animal if we have enough data (but not too frequently)
    if (movementHistory.length > 20 && !determinedAnimal && movementHistory.length % 5 === 0) {
        determineAnimal();
    }
}
...
function determineAnimal() {
    // Validate that animalDatabase exists
    if (!window.animalDatabase || !Array.isArray(window.animalDatabase)) {
        console.error('Animal database not found or invalid');
        return;
    }
    ...
    for (const animal of window.animalDatabase) {
...
function checkRevealSilhouette() {
    ...
    if (clearPercentage > revealThreshold && !silhouetteAnalyzed) {
        ...
        if (determinedAnimal) {
            ...
            // Show prompt text above button
            document.getElementById('button-prompt').style.visibility = 'visible';

            // Change button style to red background with white text
            const traceButton = document.getElementById('trace-button');
            traceButton.style.backgroundColor = '#FF0000';
            traceButton.style.color = '#FFFFFF';
            traceButton.style.borderColor = '#FF0000';
            traceButton.classList.add('red-button'); // Add class for special hover effect
        }
    }
}
...
function addEventListeners() {
    ...
    traceButton.addEventListener('click', () => {
        ...
        const alertMessage = window.i18n.t("alert-message");
        const notDetermined = window.i18n.t("not-determined");

        // Choose correct animal name based on language
        let animalName = "Unknown";
        if (determinedAnimal) {
            animalName = determinedAnimal[currentLang].name;
        } else {
            animalName = notDetermined;
        }

        alert(alertMessage + animalName);
    });
...
function updateUIOnLanguageChange() {
    ...
    // Reset button display based on current language
    const traceButton = document.getElementById('trace-button');
    traceButton.textContent = window.i18n.t("button");

    // Update button prompt
    document.getElementById('button-prompt').textContent = window.i18n.t("prompt");
...
function resetMovementMetrics() {
    ...
    // Hide prompt text above button (visibility only)
    document.getElementById('button-prompt').style.visibility = 'hidden';
    document.getElementById('button-prompt').style.display = 'block';

    // Reset button style
    const traceButton = document.getElementById('trace-button');
    traceButton.style.backgroundColor = '#FFF';
    traceButton.style.color = '#000';
    traceButton.style.borderColor = '#000';
    traceButton.classList.remove('red-button');
}
...
// Экспортируем функцию resetGame в глобальный объект window
...
function init() {
    ...
    // Позиционируем переключатель языков и обновляем при изменении размера окна
    ...
    // Empty text for height determination
    document.getElementById('button-prompt').style.visibility = 'hidden';
    document.getElementById('button-prompt').style.display = 'block';
